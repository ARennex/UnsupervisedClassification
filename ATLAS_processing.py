...
base_path = os.getcwd()
#filename = base_path + '/cep/ceps_aaronb.csv'

#obs_file = pd.read_csv(filename)
#obs_file=obs_file.sort_values(['objid','mjd'],ascending=True)
#print(obs_file)
#obs_file.to_csv(filename,index=False)
#id_lengths = obs_file.objid.value_counts() #I removed the column names for the next bit
...
for file_type in types_to_process:
    ...
    with open(filename, 'r') as input_file:
        for line in input_file:
            ...
            with open(file_type+'/Processed Files/output_{:08d}.csv'.format(index), 'a') as output_file:
